...
class Kullanici(Base):
    ...
    e_posta: Mapped[str] = mapped_column(
        String(255),
        unique=True,
        nullable=False,
        index=True
    ...
    profil_fotografi: Mapped[str | None] = mapped_column(
        String(500),
        nullable=True
    )
    ...
    aktif_mi: Mapped[bool] = mapped_column(
        Boolean,
        default=True,
        nullable=False
    )
    ...
    email_dogrulandi: Mapped[bool] = mapped_column(
        Boolean,
        default=False,
        nullable=False
    )
    ...
    hesap_kilit_bitis: Mapped[datetime | None] = mapped_column(
    ...
    son_giris_tarihi: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
    )
    ...
    talepler = relationship(
        "Talep",
        back_populates="olusturan",
        foreign_keys="Talep.olusturan_id",
    )
    ...
    atamalar = relationship(
        ...
        foreign_keys="Atama.personel_id",
    )


    bildirimler = relationship(
        "Bildirim",
        back_populates="kullanici",
    )


    refresh_tokenlar = relationship(
        "RefreshToken",
        backref="kullanici",
        cascade="all, delete-orphan",
    )

    login_loglari = relationship(
        "LoginLog",
        back_populates="kullanici",
        cascade="all, delete-orphan"
    )

      ...
      {gecmis.map((kayit, i) => {
        ...
        return (
          <li key={kayit.id} className="relative flex gap-4 pl-1">
            {!sonMu && (
              <span className="absolute left-[15px] top-8 h-[calc(100%-0.5rem)] w-0.5 bg-kenarlik" aria-hidden />
            )}
            <span
              className={cn(
            ...
            <div className="flex-1 pb-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-metin-ikincil">{tarihiBicimlendir(kayit.olusturulma_tarihi)}</span>
              </div>

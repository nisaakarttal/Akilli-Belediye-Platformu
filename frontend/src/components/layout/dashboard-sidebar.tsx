...
const PERSONEL_MENU: NavOgesi[] = [
  { href: "/panel", etiket: "Ana Sayfa", ikon: LayoutDashboard },
  { href: "/panel/atanan-talepler", etiket: "Atanan Talepler", ikon: ListChecks },
  { href: "/panel/harita", etiket: "Harita", ikon: MapPin },
  { href: "/panel/bildirimler", etiket: "Bildirimler", ikon: Bell },
  { href: "/panel/profil", etiket: "Profilim", ikon: User },
];
    ...
    <aside className="hidden w-64 shrink-0 flex-col bg-sidebar text-sidebar-foreground lg:flex">
      <Link href="/" className="flex h-16 items-center gap-2.5 px-5">
        ...
        <span className="font-display leading-tight">
          <span className="block text-xs font-bold tracking-tight text-white">AKILLI BELEDİYE</span>
      ...
      <nav className="flex-1 space-y-1 px-3 py-4" aria-label="Panel menüsü">
        {menu.map((oge) => {
      ...
      <div className="border-t border-white/10 p-4">
        <div className="rounded-xl bg-white/5 p-3.5 text-xs text-sidebar-foreground/70">
          <p className="font-medium text-sidebar-foreground/90">İhtiyacınız mı var?</p>
          <p className="mt-1">7/24 destek hattı: 0850 123 45 67</p>
        </div>
      </div>
    </aside>
  );
